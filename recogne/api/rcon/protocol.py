...
class HLLRconProtocol(asyncio.Protocol):
    ...
    def data_received(self, data: Union[bytes, None]):
        if self.xorkey is None:
            ...
            if self.logger:
                self.logger.debug(
                    "Incoming: (%s) %s", self._xor(d).count(b"\t"), d[:10]
                )
            ...
            if data is None:
                if self._waiter.done():
                    ...
                    if self.logger:
                        self.logger.warning(
                            "Active waiter is not being awaited, discarding additional incoming data"
                        )
                    self._buffer = None
                    return
            ...
            if not self._queue and self.logger:
                self.logger.warning(
                    "Received data but there are no waiters: `%s`", self._xor(data)
                )
        ...
        for i in range(i_max):
            ...
            try:
                data += await asyncio.wait_for(
                    self._get_waiter(), 2.0 if is_array else 1.0
                )
    ...
    async def execute(
        self, command, unpack_array=False, can_fail=False, multipart=False
    ):
        failed = True
        try:
            ...
            res = await asyncio.wait_for(
                self.receive(
                    waiter, decode=True, is_array=unpack_array, multipart=multipart
                ),
            ...
            if failed and self._failed and self._transport:
                if self.logger:
                    self.logger.info(
                        "Protocol failed two executions in a row and will be disconnected"
                    )
    ...
    def unpack_array(string: str):
        ...
        if arr_size != len(res):
            raise HLLUnpackError(
                "Expected array size %s but got %s" % (arr_size, len(res))
            )
        return res
