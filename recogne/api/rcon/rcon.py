...
import aiohttp
from rcon.exceptions import HLLConnectionError
from rcon.info.models import (
    InfoHopper,
    Player,
...
from rcon.info.types import Link
from rcon.mappings import (
    INFANTRY_ROLES,
    RECON_ROLES,
    SQUAD_LEADER_ROLES,
    TANK_ROLES,
...
from rcon.protocol import HLLRconProtocol
from src.hll_bot.utils import get_config, to_timedelta, ttl_cache
...
def target_to_players(
    target: Union[Player, Squad, Team, None],
...
async def get_name_from_steam(steamid: str, __name: str = None) -> str:
    ...
    params = dict(key=STEAM_API_KEY, steamids=steamid, format="json")
    ...
    async def wrapper(self: "HLLRcon", *args, **kwargs):
        async def reconnect():
            try:
                ...
                self.logger.exception(
                    "Failed to reconnect %r. Missed %s consecutive gathers.",
                    self,
        ...
        if self.connected:
            try:
                ...
                self._missed_gathers += 1
                self.logger.exception(
                    "Missed %s consecutive updates for %r, %s attempts left",
                    self._missed_gathers,
                    self,
                    20 - self._missed_gathers,
                )
                if (
                    self._missed_gathers in (4, 8, 12, 16, 20)
                    or self._missed_gathers % 20 == 0
                ):
                    await reconnect()
            else:
...
class HLLRcon:
    ...
    async def start(self):
        for worker in self.workers:
            await worker.stop()
            self.logger.warn(
                "Stopped leftover worker %r, possibly the source was not properly stopped.",
                worker,
            )
        ...
        results = await asyncio.gather(
            *[worker.start() for worker in self.workers[1:]], return_exceptions=True
        )
        for i, result in enumerate(results, start=1):
            worker = self.workers[i]
            if isinstance(result, Exception):
                self.logger.error(
                    "Failed to start worker %s: %s: %s",
                    worker.name,
    ...
    async def reconnect(self):
        ...
        if to_reconnect:
            await asyncio.gather(
                *[worker._create_connection() for worker in to_reconnect]
            )
            return True
    ...
    async def _fetch_server_info(self):
        ...
        if map == "Loading ''" or map.startswith("Untitled"):
            ...
            self._info.events.add(
                ServerMapChangedEvent(
                    self._info,
                    old=self._map.replace("_RESTART", ""),
                    new=map.replace("_RESTART", ""),
                )
            )
    ...
    async def __fetch_current_server_info(self):
        ...
        playerinfos = await asyncio.gather(
            *[
                self.exec_command("playerinfo %s" % playerid, can_fail=True)
                for playerid in playerids_normal.values()
            ]
        )
        for playerinfo in playerinfos:
            ...
            raw = dict()
            data = dict(team=None, unit=None, loadout=None)
            ...
            try:
                ...
                if team:
                    ...
                    if squad:
                        ...
                        data["squad"] = Link(
                            "squads", {"id": squad_id, "team": {"id": team_id}}
                        )
                ...
                scores = dict(
                    [
                        score.split(" ", 1)
                        for score in raw.get("score", "C 0, O 0, D 0, S 0").split(", ")
                    ]
                )
                map_score = {
                    "C": "combat",
        ...
        for steamid, name in playerids_problematic.items():
            data = dict(name=name, steamid=steamid, is_incompatible_name=True)
            players.append(data)
            ...
            for squad_id, squad_name in squadids.items():
                squads.append(
                    dict(
                        ...
                        players=Link(
                            "players",
                            {"squad": {"id": squad_id}, "team": {"id": team_id}},
                            multiple=True,
                        ),
                    )
                )
    ...
    def __parse_logs(self, logs: str):
                ...
                try:
                    ...
                    if log.startswith("KILL") or log.startswith("TEAM KILL"):
                        (
                            ...
                            r"KILL: (.+)\((Allies|Axis)\/(\d{17})\) -> (.+)\((Allies|Axis)\/(\d{17})\) with (.+)",
                            log,
                        ).groups()
                        e_cls = (
                            PlayerTeamkillEvent
                            if log.startswith("TEAM KILL")
                            else PlayerKillEvent
                        )
                        self._info.events.add(
                            e_cls(
                                self._info,
                        ...
                        player = self._info.find_players(
                            single=True, steamid=p2_steamid
                        )
                        if player:
                            ...
                            self.logger.warning(
                                "Could not find player %s %s", p2_steamid, p2_name
                            )

                    elif log.startswith("CHAT"):
                        channel, name, team, steamid, message = re.match(
                        ...
                        self._info.events.add(
                            PlayerMessageEvent(
                                ...
                                message=message,
                                channel=player.team.create_link()
                                if channel == "Team"
                                else player.squad.create_link(),
                            )
                        )

                    elif log.startswith("Player"):
                        name, steamid, action = re.match(
                            r"Player \[(.+) \((\d{17})\)\] (Left|Entered) Admin Camera",
                            log,
                        ).groups()
                        ...
                        if action == "Entered":
                            self._info.events.add(
                                PlayerEnterAdminCamEvent(
                                    self._info, event_time=time, player=player
                                )
                            )
                        elif action == "Left":
                            self._info.events.add(
                        ...
                        self._info.events.add(
                            ServerMatchStartedEvent(
                                self._info, event_time=time, map=map_name
                            )
                        )
                        self._state = "warmup"
                        ...
                        map_name, score = re.match(
                            r"MATCH ENDED `(.+)` ALLIED \((.+)\) AXIS", log
                        ).groups()
                        self._info.events.add(
                            ServerMatchEndedEvent(
                                self._info, event_time=time, map=map_name, score=score
                            )
                        )
                    ...
                    self.logger.exception(
                        "Failed to parse log line: [... (%s)] %s", timestamp, log
                    )
        ...
        for player in self._info.players:
            ...
            if player not in self._player_suicide_handles:
                if (expected_deaths is not None) and (
                    player.deaths - expected_deaths == 1
                ):
                    # The player may have redeployed. Let's wait a bit and check again.
                    handle = self.loop.call_later(
                        7.0, self.__check_player_suicide, player
                    )
                    ...
                    if (
                        (expected_deaths is not None)
                        and (player.deaths != 0)
                        and (player.deaths != expected_deaths)
                    ):
                        # Okay, maybe not entirely.
                        self.logger.warning(
                            "Mismatch for %s: Has %s but expected %s",
                            player.name,
                            player.deaths,
                            expected_deaths,
                        )
        ...
        for player in self._player_suicide_queue:
            self._info.events.add(
                PlayerSuicideEvent(
                    self._info,
                    event_time=self._logs_seen_time,
    ...
    def __check_player_suicide(self, player: Player):
        try:
            ...
            if expected_deaths is None:
                self.logger.warning(
                    "Expected death amount of player %s is unknown", player.name
                )
    ...
    async def ban_player(
        self,
        player: Player,
        time: Union[None, timedelta, datetime] = None,
        reason: str = "",
    ):
        ...
        if time:
            ...
            await self.exec_command(
                f'tempban {player.steamid} {hours} "{reason}" "HLU"'
            )
        else:
    ...
    async def kill_player(self, player: Player, reason: str = "") -> bool:
        return await self.exec_command(
            f'punish "{player.name}" "{reason}"', can_fail=True
        )
    ...
    async def send_direct_message(
        self, message: str, target: Union[Player, Squad, Team, None]
    ):
    ...
    async def set_max_team_imbalance(self, threshold: int):
        if threshold is not None and int(threshold) >= 0:
            if not self.info.server.settings.auto_balance_enabled:
                await self.exec_command("setautobalanceenabled on")
            await self.exec_command(f"setautobalancethreshold {threshold}")
        else:
            await self.exec_command("setautobalanceenabled off")
...
async def create_plain_transport(
    host: str,
    port: int,
    password: str,
    ...
    def protocol_factory():
        return HLLRconProtocol(loop=loop, timeout=10, logger=logger)

    try:
        _, protocol = await asyncio.wait_for(
            loop.create_connection(protocol_factory, host=host, port=port), timeout=15
        )
